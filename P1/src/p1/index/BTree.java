...
public class BTree {
	 ...
	 * @param order: Min number of elements in each node of the tree. Max is 2*order
	 * @param clustered: Index is clustered or not
	 * @param indexedColumn: The column that we are indexing on
	 * @param indexFile: The output file that we will be writing our index information to.
	 */
	public BTree(int order,boolean clustered,int indexedColumn, File indexFile, String input,int pageTracker, String tableName, String tempDir) {
		this.order=order;
		this.root=null;
...
//						System.out.println("Entered this part even though we are not suppose to");
			...
			ArrayList<Map.Entry<Integer, ArrayList<Integer>>> pointerInfo = new ArrayList<Map.Entry<Integer,ArrayList<Integer>>>(pointers.entrySet());
...
//						System.out.println("Entered this part even though we are not suppose to");
			...
			ArrayList<Map.Entry<Integer, ArrayList<Integer>>> pointerInfo = new ArrayList<Map.Entry<Integer,ArrayList<Integer>>>(pointersOne.entrySet());
...
//						System.out.println("Some error happen we should not be entering this loop all keys are unique");
			...
			ArrayList<Map.Entry<Integer, ArrayList<Integer>>> pointerInfoTwo = new ArrayList<Map.Entry<Integer,ArrayList<Integer>>>(pointersTwo.entrySet());
			BTreeIndexNode indexTwo= new BTreeIndexNode(this.order,childNodeTwo,pointerInfoTwo,this.getPageTracker());
			this.pageTracker+=1;
	...
	public ArrayList<BTreeNode> createLeafLayer(ArrayList<Map.Entry<Integer, ArrayList<TupleIdentifier>>> mapping) {
		...
		// Ok so we make an arraylist of leaf nodes at the bottom of the tree and there will be as many of
		// them that we need to fit all of the keys in the entry.
		ArrayList<BTreeNode> leafList= new ArrayList<BTreeNode>();
		
		// If more than 0 keys left then there is some leaf node to make. If the remaining is between 2d and 3d then stop
		// we need to split the number of nodes between them otherwise the second one will be underfull.
		while (remaining>0 && !((remaining>(2*this.order)) && remaining<(3*this.order))){
			...
			BTreeLeafNode leaf = new BTreeLeafNode(this.clustered,this.order,references,null,smallestValue,this.pageTracker);
			leafList.add(leaf);
		...
		// This means that it was not evenly distributed and last two nodes must share the reamaining amount of entries that are left.
		if (remaining>0) {
			int half=remaining/2;
			...
			BTreeLeafNode leaf1= new BTreeLeafNode(this.clustered,this.order,firstNode,null,firstNodeSmallestValue,pageTracker);
			leafList.add(leaf1);
			counter=counter+half;
			this.pageTracker+=1;
			...
			BTreeLeafNode leaf2= new BTreeLeafNode(this.clustered,this.order,secondNode,null,secondSmallest,pageTracker);
			this.pageTracker+=1;
			leafList.add(leaf2);
	...
	public ArrayList<Map.Entry<Integer, ArrayList<TupleIdentifier>>>retrieveSortedMapping() {
		...
		ArrayList<Map.Entry<Integer,ArrayList<TupleIdentifier>>> ret = new ArrayList<Map.Entry<Integer,ArrayList<TupleIdentifier>>>(allTupleOrderings.entrySet());
