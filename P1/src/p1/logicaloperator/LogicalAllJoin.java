...
public class LogicalAllJoin extends LogicalOperator{
	...
	// 1. Processed all the tables in pairs and then use the previous join as the left child.
	//2. In each of the joins we have the child being made and then either adding that 
	// to an existing join or making the new join
	
	// New Implementation:
	// 1. Need to use this information later in PhysicalPlanBuilder so we need all of the operators
	// 2. Given the first node we make in the PPBuilder I guess we can store that temporarily
	// and then from that we can get the TableString from that?
	
	// Expressioninfo is used to get check if the table needs conditions
	// Then we useExpressionInfoAliases to get if the joined table needs any conditons on it.
	 ...
	 * @param tableNames : The list of string names for the query: Ordering is the order of the query
	 * @param tableOperators : The operator for each of the tables in the same order as tableNames.
	 * @param conditions : The conditions for all the tables.
	 */
	public LogicalAllJoin(List<String> tableNames,List<LogicalOperator> tableOperators,HashMap<String[],ArrayList<Expression>> conditions) {
	 ...
	 * @return List<String> containing the names of all the tables that we are joining.
	 */
