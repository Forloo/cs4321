...
public class IndexScanOperator2 extends ScanOperator{
	...
	public IndexScanOperator2(String fromTable,Integer lowKey, Integer highKey,Boolean clustered, int colIdx, String indexFile) {
		super(fromTable);
		this.lowKey=lowKey;
		...
		// If the lowkey value here is null then that means that we want to get the first key value that 
		...
		if (this.lowKey==null) {
			...
			while(!(temp instanceof BTreeLeafNode)) {
				// If we are in this loop then that means the node is an index node meaning that we can cast to it
				BTreeIndexNode changed = (BTreeIndexNode) temp;
				ArrayList<Map.Entry<Integer, ArrayList<Integer>>> pointers= changed.getReferences();
				
				// Iterate through the arraylist until we find a pointer that is either lest than or equal
				// to the current lowkey value
				for(int i=0;i<pointers.size();i++) {
					// Check if we are on the last key since this is the one with two values in its pointers
				...
				// After you get the address to jump to then we need to update the current btreenode
				this.treeReader.resetBuffer();
	...
	public Tuple getNextTuple() {
		...
		// 2 actual one. Alternative check is to see if the key value that we are checking has a value that
		// is larger than the highkey
		// 2.Need to check if the current key for the node that we are checking if that is still in bound
		// if that value is in bound then move to next rid number. If the number if not in bound then move 
		// to the next key value. If the next key value will get us out of range then we need to read the next
		// possible leaf page. If the next page that we read is not a leaf node then make the currNode null
...
//			System.out.println("The value of the key is larger than the highkey value that we are given");
		...
		// If the currrid location is still lower then the length of the currValue row then that
		...
		// Move to the next key value
			...
			// If there are no more keys in this node then that means we are done with this node and that 
	...
	public void reset() {
		...
		// Update the currNode to be the currNode-> If the page that we are given is not a leaf address then that
		...
		if (node instanceof BTreeIndexNode) {
			...
			// This means that the page is a leaf node meaning that there are some values that we can get from it
			currNode=(BTreeLeafNode)node;
