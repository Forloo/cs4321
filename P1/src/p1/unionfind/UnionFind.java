...
public class UnionFind {
	...
	public static UnionFind mergeUnions(UnionFind unionOne,UnionFind unionTwo) {
		// Get the first element from the first union element and then from the second one get the list of that too
		
		// 1. Make a new unionfind to store the results
		// 2. Iterate through the first one and the second one order does not matter
		//3. Check merge conditions and if there is overlap then make something for the overlap
			...
			for(int j=0;j<unionTwo.getUnionElement().size();j++) {
				...
				if (found) {
					...
					if (overlap==-1) {
						...
						// We make a new element for merged so remove that element before adding the interavl
					...
					// If there is no element in between them then that means they are disjoint and tehn 
					...
					// If there no element in between the two then we need to add the column values from 
	...
	/**
	 * Check if there is overlapping elements in the set if we are to insert the current element in
	 * @param element 
	 * @return
	 */
