...
/**
 * A class that computes the order with minimum cost to join using dynamic
 * programming. Should only call this method for more than two relation joins.
 * 
 * @author Jinseok Oh
 *
 */
...
//			
		...
		HashMap<ArrayList<String>, Float> memoization2 = new HashMap<ArrayList<String>, Float>();
...
//		System.out.println(memoization);
	 ...
	 * Depending on whether there is an equality in the join condition or not the
	 * equation is different and this function helps differentiate that.
	 * 
	 * @param ltn left child table name
	 * @param rtn right child table name
	 * @return true if the join condition between two tables have an equality
	...
	/**
	 * Function that calculates the denominator of the intermediate join cost
	 * equation. This function is used when the join condition has an equality
	 * condition
	 * 
	...
	private float findDenominator(String[] ltn, String rtn) {
		...
		for (String[] key : allConditions.keySet()) {
			if (containTable(key, ltn[0]) && containTable(key, rtn)) {
				for (Expression exp : allConditions.get(key)) {
					// We only consider equality condition, if join condition does not contain
					// equality then cross product
...
//						}
	...
	/**
	 * calculates the vValue specified in the hand out (case 1 and case 2). The
	 * table name does not have a .columnName, this is because the v-value will be
	 * the same for all table's columns.
	 * 
...
//						System.out.println(cpy2.getFromTable());
						...
						float scale = 1;
						if ((float) (constraint.getMaxValue() - constraint.getMinValue()) == (float) Integer.MAX_VALUE
								- Integer.MIN_VALUE) {
							scale = 1;

							// simply don't scale in this case
						} else if (constraint.getMaxValue() == Integer.MAX_VALUE
								&& constraint.getMinValue() != Integer.MIN_VALUE) {
...
//		System.out.println("nameeeEe: " +name);
	 ...
	 * 
	 * @param keys       is the keys in conditions that contains the current table
	 *                   name
	...
	/**
	 * checks if the key list in stats.txt file contain the table name This function
	 * is used to get the column used for one table in join
	 * 
...
//		System.out.println(original.get(0));
	 ...
	 * 
	 * @return key value pair where key contains the order to join and value, the
	 *         min cost of join
	 */
...
//					System.out.println("is original mutative?: " + tableNames.size());
				...
				// get unused table list, try adding one by one, calculate intermediate join
				// cost
				...
				for (String[] key : memoization.keySet()) {
					if (key.length == window - 1) { // only look at window-1 sized keys, rest are irrelevant
						...
						// traverse temp and add one table by one to current key, calculate intermediate
						// join cost
